...
class ExistingFilesNewsManager:
    ...
    def get_oldest_unsent_news(self):
        ...
        # НОВОЕ (сортировка по имени файла = по дате):
        news_files.sort()  # Сортировка по алфавиту = по timestamp в имени
...
2. Текст: 5-7 предложения, только ключевые факты  
...
📝 5-7 предложения пересказа на русском
